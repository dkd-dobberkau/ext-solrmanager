...
class Tx_Solrmanager_ViewHelpers_CheckboxViewHelper extends \TYPO3\CMS\Fluid\ViewHelpers\Form\AbstractFormFieldViewHelper {
	...
	public function render($checked = NULL) {
		...
		$hiddenField = $this->renderHiddenFieldForEmptyValue('resultdocument_chbox');
	 ...
	 * @param string $checkboxid Set the id for the hidden field
	 * @return string the hidden field.
	 */
	protected function renderHiddenFieldForEmptyValue($checkboxid='') {
		...
		if (!in_array($fieldName, $hiddenFieldNames)) {
			...
			$inputField = '<input type="hidden" name="' . htmlspecialchars($fieldName) . '" value="" ';
			if(trim($checkboxid) != '') {
				$inputField .= 'id="tx_solrmanager_'.$checkboxid.'" ';
			}
			$inputField .= '/>';
			return $inputField;
		}
